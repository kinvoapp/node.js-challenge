...
describe("edit transaction", () => {
    ...
    it("Should not edit a transaction due to DB error", async () => {

        prismaMock.transactions.update.mockRejectedValue({ error: "Internal Error" })
        await expect(editTransaction(transaction)).rejects.toEqual({ error: "Internal Error" })
